...
if df is not None:
    ...
    if numeric_columns:
        ...
        chart_explanations = {
            "막대그래프": "📊 **막대그래프**: 여러 항목의 크기를 막대 길이로 비교할 때 좋아요. (예: 항목별 응답 비율 비교)",
        ...
        # 띠그래프를 위한 그룹 기준 열 선택 (있을 때만 표시)
        group_column = None
        if chart_type == "띠그래프":
            text_columns = df.select_dtypes(exclude="number").columns.tolist()
            if text_columns:
                group_column = st.selectbox(
                    "비교할 그룹을 나누는 기준 열이 있나요? (예: '학교', '지역' 등, 없으면 '없음' 선택)",
                    ["없음"] + text_columns,
                    key="group_column"
                )
                if group_column == "없음":
        ...
        if selected_columns:
            ...
            if chart_type == "막대그래프":
                ...
                if group_column:
                    # 그룹별로 비율을 계산해서 여러 개의 띠를 위아래로 그리기
                    grouped = df.groupby(group_column)[selected_columns].sum()
                    ...
                    # 그룹 기준이 없으면 전체를 하나의 띠로 표시
                    totals = df[selected_columns].sum()
                    proportions = totals / totals.sum() * 100
                    left = 0
                    for col_name, value in proportions.items():
                        ax.barh(0, value, left=left, label=f"{col_name} ({value:.1f}%)")
                        left += value
                    ax.set_yticks([])
                    ax.set_xlim(0, 100)
                    ax.set_xlabel("비율 (%)")
